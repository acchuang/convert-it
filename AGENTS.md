...
- Conversion stats (`lib/stats.ts`, `StatsPanel`) are local-only by decision: per pair, the number finished, the number failed with their error code, and the time. They are recorded only while history is on, and deleted when it's turned off. Never send them anywhere or add a remote analytics script; "Copy report" is the only way out, and the visitor starts it.
